...
async function init() {
    ...
    result = await prompts(
      [
        ...
        },
        {
          ...
          choices: [
            { title: red('vuex'), value: 'vuex@4.2.1' },
            { title: blue('vue-router'), value: 'vue-router@4.0.0' },
            { title: yellow('axios'), value: 'axios@1.7.10' }
          ]
        }
      ],
      { // 使用者中斷操作
  ...
  // 取得由命令列取得的建置參數
  const { framework, overwrite, packageName, variant, test } = result
