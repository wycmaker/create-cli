...
<script setup>
  ...
  const initialList = async () => {
    ...
    { // 指定高度 => 產生虛擬表格
      load.value = true
      const tableBody = scrollDom.value.$el.querySelector('.el-table__body-wrapper .el-scrollbar .el-scrollbar__wrap .el-scrollbar__view .el-table__body')
      const warp = scrollDom.value.$el.querySelector('.el-table__body-wrapper .el-scrollbar .el-scrollbar__wrap .el-scrollbar__view')
      if(warp) warp.style.display = 'block'
      const width = tableBody?.clientWidth

      // 將Table Body中的元素設為Relative，以便透過控制Top實現虛擬表格的作用
      let el = scrollDom.value.$el
      let bodys = el.querySelectorAll('.el-table__body-wrapper .el-scrollbar .el-scrollbar__wrap .el-scrollbar__view .el-table__body')
      bodys?.forEach(item => {
  ...
  const getDisplayList = () => {
    const wapper = scrollDom.value.$el.querySelector('.el-table__body-wrapper .el-scrollbar .el-scrollbar__wrap')
    let el = scrollDom.value.$el
    const tableBody = el.querySelectorAll('.el-table__body-wrapper .el-scrollbar .el-scrollbar__wrap .el-scrollbar__view .el-table__body')
